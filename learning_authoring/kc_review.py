...
def _candidate_metrics(
    ...
    return {
        ...
        "pages_with_kcs": sum(bool(page.kc_ids) for page in proposed.page_audit),
...
def _load_candidate(path: Path, approved: ExtractedSource) -> dict[str, Any]:
    proposed = ProposedKCSet.model_validate(read_json(path / "kc-proposed.json"))
...
let selected=Math.max(1,Math.min(source.source.page_count,Number(location.hash.slice(1))||1)),filter='all',zoom=1;
...
function render(){if(!scrollMode)renderDeck();position.textContent=`Slide ${selected} of ${source.source.page_count}`;previous.disabled=selected<=1;next.disabled=selected>=source.source.page_count;extractionLink.href=`extraction-review.html?from=kc#${selected}`;slideDeck.querySelectorAll('[data-slide-page]').forEach(x=>x.classList.toggle('active',Number(x.dataset.slidePage)===selected));renderList();renderInspector();renderFloatingKcs();requestAnimationFrame(()=>{if(!scrollMode)setZoom(zoom);pageList.querySelector('.page-row.active')?.scrollIntoView({block:'nearest'})})}
...
function selectPage(page,shouldScroll=false,behavior='smooth'){const nextPage=Math.max(1,Math.min(source.source.page_count,page));if(nextPage!==selected){selected=nextPage;history.replaceState(null,'',`#${selected}`);render()}if(shouldScroll)scrollToPage(selected,behavior)}
...
sourceSummary.textContent=`${source.source.page_count} slides · ${kcs.length} Leaf KCs · ${groupList.length} groups`;sidebarToggle.onclick=()=>togglePanel('left');sidebarCollapse.onclick=()=>togglePanel('left');inspectorToggle.onclick=()=>togglePanel('right');previous.onclick=()=>selectPage(selected-1,true);next.onclick=()=>selectPage(selected+1,true);zoomOut.onclick=()=>setZoom(zoom-.1);zoomIn.onclick=()=>setZoom(zoom+.1);fit.onclick=()=>setZoom(1);search.oninput=renderList;const trackScroll=()=>{if(scrollMode&&scrollTracking&&!scrollFrame)scrollFrame=requestAnimationFrame(syncPageFromScroll)};addEventListener('scroll',trackScroll,{passive:true});slideScroll.addEventListener('scroll',trackScroll,{passive:true});addEventListener('keydown',e=>{if(e.target.matches('input,textarea'))return;if(e.key==='ArrowLeft')selectPage(selected-1,true);if(e.key==='ArrowRight')selectPage(selected+1,true)});addEventListener('hashchange',()=>selectPage(Number(location.hash.slice(1))||1,true));addEventListener('resize',()=>setZoom(zoom));renderGroupFilters();renderDeck();updatePanelToggles();render();Promise.all([...slideDeck.querySelectorAll('img')].map(img=>img.complete?Promise.resolve():new Promise(resolve=>{img.onload=resolve;img.onerror=resolve}))).then(()=>{setZoom(1);if(scrollMode)requestAnimationFrame(()=>{selected=Math.max(1,Math.min(source.source.page_count,Number(location.hash.slice(1))||1));render();setTimeout(()=>{scrollToPage(selected,'auto');requestAnimationFrame(()=>{scrollTracking=true})},120)})});
</script></body></html>'''
    return (
...
const metricRows=[['Contract valid','contract_valid'],['Leaf KCs','leaf_kcs'],['Groups','groups'],['Pages with KCs','pages_with_kcs'],['Evidence records','evidence_records'],['Referenced source blocks','referenced_source_blocks'],['Block reference rate','block_reference_rate','pct'],['Multi-page KCs','multi_page_kcs'],['Conditional claim rate','conditional_claim_rate','pct'],['Warnings','warnings'],['Uncovered items','uncovered_items'],['Input tokens','input_tokens'],['Cached input tokens','cached_input_tokens'],['Output tokens','output_tokens'],['Reasoning tokens','reasoning_tokens'],['Total tokens','total_tokens'],['Model time (s)','model_seconds'],['Reported cost (USD)','cost_usd']];
...
def build_kc_demo(
    ...
    *,
    ...
    candidates = [_load_candidate(path, approved) for path in paths]
    if not candidates:
