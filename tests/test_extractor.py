...
def test_extractor_writes_proposed_not_approved(tmp_path, source, monkeypatch) -> None:
    monkeypatch.setattr(
        "learning_authoring.extractor.prepare_or_reuse_source", fake_source_prep(source)
    )
...
def test_repair_guard_config_reaches_policy_fingerprint_and_metadata(
    ...
    monkeypatch.setattr("learning_authoring.extractor.run_repairs", capture_repairs)
    pdf = tmp_path / "input.pdf"
    pdf.write_bytes(b"ignored")
    run_dir = tmp_path / "run"
    settings = replace(
